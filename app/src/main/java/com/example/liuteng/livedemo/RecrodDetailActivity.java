...
import com.example.liuteng.livedemo.util.ConfigUtil;
import com.example.liuteng.livedemo.util.DataSet;
import com.example.liuteng.livedemo.util.DateUtil;
import com.example.liuteng.livedemo.util.MediaUtil;
import com.example.liuteng.livedemo.util.ParamsUtil;
import com.example.liuteng.livedemo.util.XlfLog;
import com.example.liuteng.livedemo.view.PlayChangeVideoPopupWindow;
import com.example.liuteng.livedemo.view.PlayTopPopupWindow;
import com.example.liuteng.livedemo.view.PopMenu;
import com.example.liuteng.livedemo.view.Subtitle;
import com.example.liuteng.livedemo.view.TitleView;
import com.example.liuteng.livedemo.view.VerticalSeekBar;
...
public class RecrodDetailActivity extends BaseActivity implements
        ...
        MediaPlayer.OnVideoSizeChangedListener, SurfaceHolder.Callback, SensorEventListener, MediaPlayer.OnCompletionListener {
    ...
    private RecordBelowAdapter mAdapter;
    ...
    private void initLoadMore() {
        ...
        mLoadMoreWrapper.setLoadMoreView(LayoutInflater.from(this).inflate(R.layout.default_loading, rvBelowInfo, false));
    ...
    private void noMoreData() {
        mhandler.post(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(RecrodDetailActivity.this, "没有更多数据了", Toast.LENGTH_LONG).show();
    ...
    private void showMobileDialog() {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                AlertDialog.Builder builder = new AlertDialog.Builder(RecrodDetailActivity.this);
    ...
    private void initViews() {
        ...
        rvBelowInfo = (RecyclerView) findViewById(R.id.rv_below_info);
    ...
    private void initPlayInfo() {
        ...
        videoIdText.setText(recordID);
        isLocalPlay = getIntent().getBooleanExtra("isLocalPlay", false);
        try {

            if (!isLocalPlay) {// 播放线上视频
                player.setVideoPlayInfo(recordID, ConfigUtil.USERID, ConfigUtil.API_KEY, this);
                // 设置默认清晰度
                ...
                if (android.os.Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
                    path = Environment.getExternalStorageDirectory() + "/".concat(ConfigUtil.DOWNLOAD_DIR).concat("/").concat(recordID).concat(".mp4");
            ...
            XlfLog.e("player error" + e.getMessage());
        } catch (SecurityException e) {
            XlfLog.e("player error" + e.getMessage());
        } catch (IllegalStateException e) {
            XlfLog.e("player error" + e + "");
        }
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        try {
            ...
            XlfLog.e("videoPlayer" + "error" + e);
        }
        XlfLog.i("videoPlayer" + "surface+created");
    }
    ...
    public void onPrepared(MediaPlayer mp) {
        ...
        if (!isLocalPlay) {
            if (currentPosition > 0) {
                ...
                lastPlayPosition = DataSet.getVideoPosition(recordID);
...
//			firstInitDefinition = false;
    ...
    private void changeToNextVideo(boolean isCompleted) {
        ...
        int length = RecordRadioFragment.playVideoIds.length;
        int position = 0;
    ...
    private int getCurrentVideoIndex() {
        return Arrays.asList(RecordRadioFragment.playVideoIds).indexOf(recordID);
    }
    ...
    private void changeVideo(int position, boolean isCompleted) {
        ...
        recordID = RecordRadioFragment.playVideoIds[position];
        ...
        player.reset();
        player.setDefaultDefinition(defaultDefinition);
        player.setVideoPlayInfo(recordID, ConfigUtil.USERID, ConfigUtil.API_KEY, RecrodDetailActivity.this);
    ...
    private ServiceConnection serviceConnection = new ServiceConnection() {
        ...
        public void onServiceDisconnected(ComponentName name) {
            XlfLog.i("service disconnected" + name + "");
        }
    ...
    private void downloadCurrentVideo() {
        if (DataSet.hasDownloadInfo(recordID)) {
            Toast.makeText(this, "文件已存在", Toast.LENGTH_SHORT).show();
            return;
        }

        File file = MediaUtil.createFile(recordID);
        if (file == null) {
            Toast.makeText(this, "创建文件失败", Toast.LENGTH_SHORT).show();
            return;
        }

        DataSet.addDownloadInfo(new DownloadInfo(recordID, recordID, 0, null, Downloader.WAIT, new Date()));

        Intent service = new Intent(this, DownloadService.class);
        service.putExtra("title", recordID);
    ...
    private void updateDataPosition() {
        ...
        if (currentPlayPosition > 0) {
            if (DataSet.getVideoPosition(recordID) > 0) {
                DataSet.updateVideoPosition(recordID, currentPlayPosition);
            } else {
                DataSet.insertVideoPosition(recordID, currentPlayPosition);
            }
        }
    }
